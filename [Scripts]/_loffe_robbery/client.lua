...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
    ...
    TriggerServerEvent('esx_addons_gcphone:startCall', 'police', 'Braquage en cours', {
    x = PedPosition.x, y = PedPosition.y, z = PedPosition.z})
    ...
    if not IsPedDeadOrDying(peds[i]) then
        ...
        if not IsPedDeadOrDying(peds[i]) then
            ...
            if not IsPedDeadOrDying(peds[i]) then
                ...
                rand =math.random(120,160)
                sacBase = 0
                Citizen.CreateThread(function()
                    while true do
                        Wait(500)
                        if rand > sacBase then
                ...
                TriggerEvent("loffe_robbery:removePickup", -1, i)
            else
                DeleteObject(bag)
            end
        end
