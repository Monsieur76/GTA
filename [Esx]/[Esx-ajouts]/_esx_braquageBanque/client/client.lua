...
AddEventHandler('esx_holdupbank:hackcomplete', function()
	...
	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', 'Braquage en cours', { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z})
	ClearPedTasksImmediately(PlayerPedId())
...
AddEventHandler('esx_holdupbank:plantedbomb', function(x,y,z,doortype)
    ...
    AddExplosion( x,  y, z , 17, 5, 1, 0, 1065353216, 10)
    AddExplosion( x,  y, z , 17, 5, 1, 0, 1065353216, 10)
   -- AddExplosion( bank.bombposition.x,  bank.bombposition.y, bank.bombposition.z , 0, 0.5, 1, 0, 1065353216, 0)
...
AddEventHandler('esx_holdupbank:exit', function(bank)
	SetEntityCoordsNoOffset(PlayerPedId(), bank.hackposition.x , bank.hackposition.y, bank.hackposition.z, 0, 0, 1)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Banks)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not hackholdingup then
					DrawMarker(27, v.hackposition.x, v.hackposition.y, v.hackposition.z - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0,255, 0, 0, 0,0)
		...
		if hackholdingup then
			...
			RequestAnimDict("anim@heists")
			RequestAnimDict("anim@heists@ornate_bank")
			...
			TaskPlayAnim(playerPed,"anim@heists@ornate_bank@hack","hack_loop",1.0,-1.0, 5000, 0, 1, true, true, true)
			...
			if (pos2 ~= nil) then
				if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
					if not bombholdingup then
						DrawMarker(27, v.bombposition.x, v.bombposition.y, v.bombposition.z - 1, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0, 0,255, 0, 0, 0,0)
...
RegisterNetEvent('esx_holdupbank:VoleArgent')
AddEventHandler('esx_holdupbank:VoleArgent', function()
	PlaySoundFrontend(-1, "Bus_Schedule_Pickup", "DLC_PRISON_BREAK_HEIST_SOUNDS", 0)
	Citizen.Wait(100)
	ESX.ShowAdvancedNotification('BRAQUAGE', 'BRAQUAGE EN COURS.', 'Vous avez pris de l\'argent dans votre sac', 'CHAR_LESTER_DEATHWISH', 1)
end)
