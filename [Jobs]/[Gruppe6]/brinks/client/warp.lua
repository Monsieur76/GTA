...
local position = {
	{name= "Rez-de-chaussée", x = -15.97, y = -699.11, z = 45.02 },
	{name= "-1", x = 14.59, y = -691.33, z = 39.73 },
	{name= "-2", x = 10.26, y = -668.12, z = 32.45 }
}
...
Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('ascenseur', 'main'), true, true, true, function()
			RageUI.ButtonWithStyle("Rez-de-chaussée", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
				if (Selected) then  
			...
			RageUI.ButtonWithStyle("-1", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
				if (Selected) then  
					local playerPed = PlayerPedId()
			...
			RageUI.ButtonWithStyle("-2", nil, {RightLabel = "→→→"}, true, function(Hovered, Active, Selected)
				if (Selected) then  
					local playerPed = PlayerPedId()
