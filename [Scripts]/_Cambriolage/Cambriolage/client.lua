...
RMenu:Get("cambriolage", 'script_main').Closed = function()
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        RageUI.IsVisible(RMenu:Get("cambriolage", "script_main"), true, true, true, function()
            ...
            RageUI.ButtonWithStyle("Entrer dans la maison", "Forcer la serrure", {
                RightLabel = ""
            }, not cooldown, function(h, a, s)
                if s then
                    RageUI.CloseAll()
                    cooldown = true
                    Citizen.SetTimeout(1800000, function()
                    end)
                    SetEntityCoords(PlayerPedId(), 117.4625, 559.5997, 184.3048, false, false, false, true)
                    ESX.ShowAdvancedNotification("Cambriolage ", "~r~Cambriolage !",
                        "Bonne chance et n'oublie pas que tout ce paye !", "CHAR_LESTER_DEATHWISH", 24, 5)
                end
            end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(position) do
            ...
            if dist <= 5.5 then
                ESX.ShowHelpNotification("Faite vite et attention aux empreintes")
