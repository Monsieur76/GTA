...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Vous avez acheté ~y~" .. tattooName .. "~s~ pour ~g~$" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Vous n'avez pas assez pour acheter ce tattoo")
		cb(false)
	end
end)
