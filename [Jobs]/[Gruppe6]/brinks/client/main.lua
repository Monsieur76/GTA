...
AddEventHandler('esx_gruppe6:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'brinks' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'brinks' and not IsDead then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'brinks' then
		...
		if IsControlJustReleased(0, 167) and not isDead and PlayerData.job and PlayerData.job.name == 'brinks' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'security_actions') then
			OpenSecurityActionsMenu()
		end
		
		if IsControlPressed(0, 32) and IsControlJustReleased(0, 46) and not isDead and PlayerData.job and PlayerData.job.name == 'brinks' then
			GiveUp()
		end
		
		if IsControlPressed(0, 33) and IsControlJustReleased(0, 46) and not isDead and PlayerData.job and PlayerData.job.name == 'brinks' then
			Release()
		end
	end
end)
...
AddEventHandler('esx_gruppe6:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'brinks' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'brinks' then
