function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords,
    ...
    self.getCanne = function()
        return self.canne
    end

    self.setCanne = function()
        if self.canne == false then
            self.canne = true
        else
            self.canne = false
        end
    ...
    self.getMaladie = function()
        return self.maladie
    end

    self.setMaladie = function(maladie)
        table.insert(self.maladie, maladie)
        return self.maladie
    end

    self.removeMaladie = function(maladie)
        for k, v in ipairs(self.maladie) do
            if maladie == v then
                table.remove(self.maladie, k)
            end
        end
        return self.maladie
    end

    self.getDiplomas = function()
        return self.diplomas
    end

    self.setDiplomas = function(diploma)
        table.insert(self.diplomas, diploma)
        return self.diplomas
    end
    ...
    self.addWeapon = function(weaponName, ammo)
        if not self.hasWeapon(weaponName) then
            ...
            self.triggerEvent('esx:addWeapon', weaponName, ammo ,weaponLabel)
