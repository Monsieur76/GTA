...
Citizen.CreateThread(function()
	while true do
		...
		local bool , weaponHash=GetCurrentPedWeapon(playerPed,1 )
		-- is jackin'
		...
		elseif IsPedInMeleeCombat(playerPed) and Config.MeleeAlert and IsPedArmed(playerPed,7) then
		...
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert and bool == 1 and  weaponHash ~= 911657153 then

			Citizen.Wait(3000)

			if ((isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted) and DecorGetInt(PlayerPedId(), 'isOutlaw') ~= 2 then
				DecorSetInt(playerPed, 'isOutlaw', 2)
