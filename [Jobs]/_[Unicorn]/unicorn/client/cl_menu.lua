...
RMenu.Add('unicorn', 'main', RageUI.CreateMenu("Unicorn", "Intéraction"))
RMenu.Add('unicorn', 'inter', RageUI.CreateMenu("Unicorn", "Intéraction"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('unicorn', 'main'), true, true, true, function()
	...
	RageUI.IsVisible(RMenu:Get('unicorn', 'inter'), true, true, true, function()
...
Citizen.Wait(0)
    ...
    Citizen.CreateThread(function()
        ...
        --    RegisterNetEvent('esx_unicornjob:onDuty')
            if IsControlJustReleased(0 ,167) then
                RageUI.Visible(RMenu:Get('unicorn', 'main'), not RageUI.Visible(RMenu:Get('unicorn', 'main')))
            end
        end
        end
    end)
    ...
    RageUI.Visible(RMenu:Get('unicorn', 'main'), not RageUI.Visible(RMenu:Get('unicorn', 'main')))
		...
		function demenotter()
            ...
            TriggerServerEvent('esx_unicornjob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
            Wait(5000)
			TriggerServerEvent('esx_unicornjob:handcuff', GetPlayerServerId(closestPlayer))
		else
...
local blips = {
	-- Example {title="", colour=, id=, x=, y=, z=},
	 ...
	 {title="Unicorn Club", colour=48, id=121, x = 130.44, y = -1301.1, z = 17.53},
