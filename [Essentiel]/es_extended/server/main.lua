...
Citizen.CreateThread(function()
    ...
    local query =
        '`accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `firstname`, `lastname`, `dateofbirth`, `sex`, `height`' -- Select these fields from the database
    ...
    MySQL.Async.store("SELECT * FROM `users` WHERE identifier = ?", function(storeId)
        LoadPlayer = storeId
    end)
end)
...
function createESXPlayer(identifier, playerId, data)
    ...
    if not Config.Multichar then
        MySQL.Async.execute(NewPlayer, {json.encode(accounts), identifier, defaultGroup}, function(rowsChanged)
        ...
        MySQL.Async.execute(NewPlayer,
            {json.encode(accounts), identifier, defaultGroup, data.firstname, data.lastname, data.dateofbirth, data.sex,
             data.height}, function(rowsChanged)
                loadESXPlayer(identifier, playerId, true)
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    table.insert(tasks, function(cb)
        MySQL.Async.fetchAll(LoadPlayer, {identifier}, function(result)
            local job, grade, jobObject, gradeObject = result[1].job, tostring(result[1].job_grade)
            local foundAccounts, foundItems, foundDiplomas = {}, {}, {}
            ...
            for name, item in pairs(ESX.Items) do
                ...
                table.insert(userData.inventory, {
                    ...
                    vente = ESX.SellItemsCallbacks[name] ~= nil,
                    rare = item.rare,
                    legal = item.legal,
            ...
            if result[1].group then
                if result[1].group == "admin" then
                    ...
                    ExecuteCommand('add_principal identifier.'..identifier..' group.admin')
                else
            ...
            userData.canne = result[1].canne

            if result[1].maladie ~= '' then
                userData.maladie = json.decode(result[1].maladie)
            end

            if result[1].phone ~= '' then
                userData.phone = result[1].phone_number
            end

            if result[1].maxWeight ~= '' then
                userData.maxWeight = result[1].maxWeight
            end

            -- Diplomas
            if result[1].diplomas and result[1].diplomas ~= '' then
                userData.diplomas = json.decode(result[1].diplomas)
            end
    ...
    Async.parallel(tasks, function(results)
        local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts,
            userData.inventory, userData.weight, userData.job, userData.loadout, userData.playerName, userData.coords,
            userData.canne, userData.maladie, userData.diplomas,userData.phone)
        ...
        xPlayer.triggerEvent('esx:playerLoaded', {
            ...
            dead = false,
            canne = xPlayer.getCanne(),
            maladie = xPlayer.getMaladie(),
            diplomas = xPlayer.getDiplomas(),
...
AddEventHandler('chatMessage', function(playerId, author, message)
    if message:sub(1, 1) == '/' and playerId > 0 then
        ...
        TriggerClientEvent('chat:addMessage', playerId, {
            args = {'^1SYSTEM', _U('commanderror_invalidcommand', commandName)}
        })
    end
end)
    ...
    elseif type == 'item_account' then
        if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
            ...
            if itemName == "black_money" then
                sourceXPlayer.showNotification('Vous avez donné ~r~' .. ESX.Math.GroupDigits(itemCount) .. "$")
            else
                sourceXPlayer.showNotification('Vous avez donné ~g~' .. ESX.Math.GroupDigits(itemCount) .. "$")
            end

            -- targetXPlayer.showNotification('Vous avez reçue ~g~' .. ESX.Math.GroupDigits(itemCount) .. "$")
        else
    ...
    elseif type == 'item_account' then
        if itemCount == nil or itemCount < 1 then
            ...
            if (itemCount > account.money or account.money < 1) then
                ...
                local pickupLabel = ('~y~%s~s~ [~g~%s~s~]'):format(account.label, _U('locale_currency',
...
RegisterNetEvent('esx:SellItem')
AddEventHandler('esx:SellItem', function(itemName)
    local xPlayer = ESX.GetPlayerFromId(source)
    local count = xPlayer.getInventoryItem(itemName).count

    if count > 0 then
        ESX.SellItem(source, itemName)
    else
        xPlayer.showNotification(_U('act_imp'))
    end
end)
...
function loadvetement(identifier, playerId, isNew)
    local tasks = {}
    ...
    table.insert(tasks, function(cb)
        MySQL.Async.fetchAll("SELECT * FROM clothe_society", function(result)

            userData.tenue = json.decode(result[1].tenue)
            userData.label = result[1].label

            userData.job= result[1].job
            userData.sex = result[1].sex
