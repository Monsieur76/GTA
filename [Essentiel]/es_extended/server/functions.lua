...
Citizen.CreateThread(function()
	savePlayers = MySQL.Sync.store("UPDATE users SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `canne` = ?, `maladie` = ?,`MaxWeight` = ? WHERE `identifier` = ?")
end)
...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb2)
		MySQL.Async.execute(savePlayers, {
			...
			xPlayer.getCanne(),
			json.encode(xPlayer.getMaladie()),
...
ESX.RegisterSellItem = function(item, cb)
	ESX.SellItemsCallbacks[item] = cb
end

ESX.SellItem = function(source, item)
	ESX.SellItemsCallbacks[item](source, item)
end
